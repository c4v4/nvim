    ...
    if vim.api.nvim_win_is_valid(win) then
      ...
      if not vim.b[buf].claudecode_diff_tab_name then
        ...
        elseif not has_proposed then
          ...
          if ok and wb and (wb:match("^%[ PROPOSED %]") or wb:match("^%[ ORIGINAL %]")) then
            set_winbar(win, "")
          end
        end
      end
...
vim.api.nvim_create_autocmd({ "BufWinEnter", "WinEnter" }, {
  ...
  callback = function() vim.schedule(update_claudecode_diff_winbars) end,
})
...
require("lazy").setup({
    ...
    config = function()
        ...
        builtin.find_files({
          ...
          attach_mappings = function(prompt_bufnr, map)
            ...
            local function open_dir_picker()
              ...
              vim.schedule(function() directory_picker_for_mode(path) end)
            end
      ...
      local function smart_live_grep(opts)
        ...
        builtin.live_grep({
          ...
          additional_args = function() return additional_args end,
          attach_mappings = function(prompt_bufnr, map)
            -- <C-b>: Open directory picker
            local function open_dir_picker()
            ...
            map("i", "<C-f>", function()
              ...
              if filter_input == "" then return end
    ...
    config = function(_, opts)
      ...
      vim.keymap.set({ "n", "t" }, "<C-\\>", function()
        if not term then
          ...
          if vim.fn.isdirectory(cwd) == 0 then cwd = vim.fn.getcwd() end
  ...
  -- -------------------------------------------------------------------------
  {
    ...
    config = function()
      ...
      vim.keymap.set("n", "<leader>gd", function()
        local ctx = get_buffer_context()
        if ctx.git_root then vim.cmd.cd(ctx.git_root) end
        vim.cmd.Gdiffsplit()
      end, { desc = "Git diff" })
  ...
  -- cancel_deferred_save (InsertEnter): cancel the pending save when typing resumes,
