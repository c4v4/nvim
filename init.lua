...
require('lazy').setup {
  ...
  -- Treesitter (syntax highlighting, smart selection)
  {
    ...
    build = ':TSUpdate',
    opts = {
      ensure_installed = { 'c', 'cpp', 'python', 'lua', 'vim', 'bash', 'cmake', 'json', 'yaml', 'make', 'markdown', 'markdown_inline' },
      auto_install = true,
      highlight = {
        enable = true,
  ...
  -- Fuzzy Finder
  {
    ...
    config = function()
      ...
      local function smart_find_files(scope, query)
        ...
        local find_command = vim.fn.executable 'fd' == 1 and { 'fd', '--type', 'f', '--hidden', '--no-ignore-vcs', '--exclude', '.git', '--exclude', 'build' }
          or nil
      ...
      vim.keymap.set('n', '<leader>sS', builtin.lsp_dynamic_workspace_symbols, { desc = 'Search symbols (project)' })
